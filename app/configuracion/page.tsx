...
const settings: Setting[] = [
  { title: "Mi cuenta", description: "Tu perfil, datos personales y preferencias de acceso.", icon: "user", tone: "text-blue-700" },
  { title: "Empresa", description: "Información general y datos de contacto de tu empresa.", icon: "building", tone: "text-emerald-700" },
  { title: "Usuarios y permisos", description: "Administra usuarios, roles y permisos de acceso.", icon: "users", tone: "text-violet-700", action: "permissions" },
  { title: "Equipos", description: "Categorías, marcas, modelos y estados de los equipos.", icon: "equipment", tone: "text-blue-700" },
  { title: "Inventario", description: "Repuestos, categorías, stock mínimo y proveedores.", icon: "inventory", tone: "text-amber-700" },
  { title: "Mantenimiento", description: "Intervalos, prioridades y reglas de mantenimiento.", icon: "wrench", tone: "text-orange-700" },
  { title: "Notificaciones", description: "Alertas internas, canales de comunicación y recordatorios.", icon: "bell", tone: "text-emerald-700", action: "notifications" },
  { title: "Integraciones", description: "Conecta ClimaControl con otras herramientas y servicios.", icon: "link", tone: "text-violet-700" },
  { title: "Documentos", description: "Plantillas, informes, certificados y documentos del sistema.", icon: "file", tone: "text-rose-700" },
  { title: "Datos", description: "Importa, exporta y realiza copias de seguridad.", icon: "database", tone: "text-blue-700" },
  { title: "Seguridad", description: "Autenticación y políticas de seguridad.", icon: "shield", tone: "text-violet-700" },
  { title: "Auditoría", description: "Consulta el historial de actividad del sistema.", icon: "history", tone: "text-amber-700" },
  { title: "Plan y facturación", description: "Información de plan, facturación y métodos de pago.", icon: "card", tone: "text-blue-700" },
  { title: "Acerca de", description: "Versión del sistema, términos de uso y privacidad.", icon: "info", tone: "text-slate-600" },
]
...
export default function ConfiguracionPage() {
  ...
  return <div className="mx-auto max-w-[1260px] px-5 py-7 text-slate-900 sm:px-7 lg:px-9">
    ...
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{settings.map((setting) => <button key={setting.title} type="button" onClick={() => setting.action ? setSelected(setting.action) : setNotice(`${setting.title}: próximamente.`)} className="group min-h-[154px] rounded-xl border border-slate-200 bg-white p-5 text-left transition-colors hover:border-slate-300 hover:bg-slate-50"><span className={`inline-flex ${setting.tone}`}><SettingIcon name={setting.icon} /></span><div className="mt-4 flex items-center justify-between gap-3"><h2 className="text-base font-semibold tracking-[-.02em] text-slate-900">{setting.title}</h2><svg aria-hidden="true" className="h-5 w-5 text-slate-400 transition-colors group-hover:text-blue-700" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="m9 18 6-6-6-6" /></svg></div><p className="mt-1.5 max-w-[30ch] text-sm leading-5 text-slate-500">{setting.description}</p>{!setting.action && <span className="mt-3 inline-block text-xs font-medium text-slate-400">Próximamente</span>}</button>)}</section>
  </div>
}
