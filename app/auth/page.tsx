...
export default function AuthPage() {
  ...
  return <div className="min-h-screen w-full bg-[radial-gradient(circle_at_1px_1px,#d8e4f4_1px,transparent_1px)] bg-[size:16px_16px] md:ml-[-280px] md:w-[calc(100%+280px)] lg:grid lg:grid-cols-[minmax(420px,37%)_1fr]">
    ...
    <main className="flex min-h-screen items-center justify-center p-6 sm:p-10 xl:p-14"><div className="w-full max-w-[610px] rounded-2xl border border-slate-200 bg-white px-7 py-11 shadow-[0_20px_55px_rgba(15,42,82,.10)] sm:px-12 sm:py-14"><div className="mx-auto grid h-17 w-17 place-items-center rounded-2xl bg-gradient-to-br from-[#2784ff] to-[#0958c9] text-white shadow-[0_12px_26px_rgba(8,94,205,.28)]"><Snowflake className="h-10 w-10" /></div><div className="mt-7 text-center"><h2 className="text-[28px] font-bold tracking-[-.045em] text-slate-900">{activationMode ? "Activa tu acceso" : "Bienvenido de nuevo"}</h2><p className="mt-2 text-[15px] text-slate-500">{activationMode ? "Configura tu código una sola vez" : "Ingresa con tu email y código personal"}</p></div>
      {loading || processingLink ? <p className="mt-11 text-center text-sm text-slate-500">Validando sesión…</p> : user ? <div className="mt-11 space-y-5"><div className="rounded-xl border border-blue-100 bg-blue-50 px-5 py-4"><p className="text-sm text-slate-600">Sesión activa como</p><p className="mt-1 font-bold text-slate-900">{displayName}</p><p className="text-sm text-slate-500">{user.email}</p></div><div className="flex flex-wrap gap-3"><button type="button" onClick={signOut} className="rounded-lg bg-blue-600 px-5 py-3 text-[13px] font-semibold text-white hover:bg-blue-700">Cerrar sesión</button><Link href="/" className="rounded-lg border border-slate-200 px-5 py-3 text-[13px] font-semibold text-slate-700 hover:bg-slate-50">Ir al Panel</Link></div></div> : activationMode ? <form onSubmit={handleMagicLink} className="mt-11 space-y-7"><div><label htmlFor="fullName" className="text-[13px] font-semibold text-slate-800">Nombre para mostrar</label><input id="fullName" type="text" value={fullName} onChange={(event) => setFullName(event.target.value)} placeholder="Ej: Ángel" autoComplete="name" className="mt-2 w-full rounded-lg border border-slate-300 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100" /></div><div><label htmlFor="email" className="text-[13px] font-semibold text-slate-800">Correo electrónico</label><input id="email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} required autoComplete="email" placeholder="tu@empresa.com" className="mt-2 w-full rounded-lg border border-slate-300 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100" /></div>{codeInput}<Status message={message} error={error} /><button type="submit" disabled={sending || cooldownLeft > 0} className="w-full rounded-lg bg-gradient-to-r from-blue-600 to-blue-700 px-5 py-3 text-[13px] font-semibold text-white shadow-[0_8px_18px_rgba(37,99,235,.18)] transition hover:from-blue-700 hover:to-blue-800 disabled:opacity-70">{sending ? "Enviando…" : cooldownLeft > 0 ? `Reenviar en ${cooldownLeft}s` : "Recibir enlace de activación"}</button><button type="button" onClick={() => { setActivationMode(false); setError(""); setMessage("") }} className="w-full text-[13px] font-semibold text-slate-500 hover:text-blue-700">Ya tengo un código</button></form> : <form onSubmit={handleCodeSignIn} className="mt-11 space-y-7"><div><label htmlFor="email" className="text-[13px] font-semibold text-slate-800">Correo electrónico</label><input id="email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} required autoComplete="email" placeholder="tu@empresa.com" className="mt-2 w-full rounded-lg border border-slate-300 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100" /></div>{codeInput}<Status message={message} error={error} /><button type="submit" disabled={signingInWithCode} className="w-full rounded-lg bg-gradient-to-r from-blue-600 to-blue-700 px-5 py-3 text-[13px] font-semibold text-white shadow-[0_8px_18px_rgba(37,99,235,.18)] transition hover:from-blue-700 hover:to-blue-800 disabled:opacity-70">{signingInWithCode ? "Ingresando…" : "Iniciar sesión"}</button><div className="flex items-center gap-3 text-xs text-slate-400"><span className="h-px flex-1 bg-slate-200" />¿Primera vez? <span className="h-px flex-1 bg-slate-200" /></div><button type="button" onClick={() => { setActivationMode(true); setError(""); setMessage("") }} className="w-full rounded-lg border border-slate-200 px-5 py-3 text-[13px] font-semibold text-blue-700 transition hover:bg-blue-50">Activar mi acceso por email</button></form>}</div></main>
  </div>
}
