...
export default function TramiteDetalle() {
  ...
  useEffect(() => {
    ...
    window.setTimeout(() => document.querySelector('textarea[name="descripcion"]')?.focus(), 0)
    ...
    return () => {
      ...
      window.setTimeout(() => editButton?.focus() || previouslyFocused?.focus?.(), 0)
    }
  }, [closeEditor, editing])
  ...
  const save = async (event) => {
    ...
    if (data) { setTramite((previous) => ({ ...previous, ...data })); closeEditor() }
    setSaving(false)
  }

  if (loading) return <div className="grid min-h-[50vh] place-items-center"><span className="h-9 w-9 animate-spin rounded-full border-2 border-slate-200 border-t-blue-600" /></div>
  ...
  return <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    <div className="mb-7 flex flex-col gap-5 border-b border-slate-200 pb-6 sm:flex-row sm:items-end sm:justify-between">
      <div className="flex items-start gap-3">
        ...
        <div><p className="mb-1 text-xs font-semibold uppercase tracking-[0.16em] text-blue-600">{isMaintenance ? "Mantenimiento" : "Abono"}</p><h1 className="text-3xl font-bold tracking-tight text-slate-950">Detalle del trámite</h1><p className="mt-1.5 text-sm text-slate-500">Gestiona su programación, importe y estado.</p></div>
      </div>
      <button ref={editButtonRef} onClick={() => setEditing(true)} className="inline-flex min-h-11 w-full items-center justify-center rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm shadow-blue-600/20 transition hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 sm:w-auto">Editar trámite</button>
    </div>
    ...
    <section className="mb-6 grid gap-3 sm:grid-cols-3">
      <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs font-medium text-slate-500">Estado actual</p><span className={`mt-3 inline-flex rounded-full px-3 py-1 text-sm font-semibold ring-1 ${status}`}>{statusNames[tramite.estado] || "Pendiente"}</span></article>
      <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs font-medium text-slate-500">Fecha programada</p><p className="mt-2 text-sm font-semibold capitalize text-slate-900">{localDate(tramite.fecha_programada)}</p></article>
      <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs font-medium text-slate-500">Importe</p><p className="mt-2 text-xl font-bold tracking-tight text-slate-900">{amount}</p></article>
    </section>
    <div className="grid gap-6 lg:grid-cols-[1.25fr_.75fr]">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><h2 className="text-lg font-semibold text-slate-950">Información del servicio</h2><dl className="mt-5 divide-y divide-slate-100"><div className="py-4 first:pt-0"><dt className="text-sm text-slate-500">Descripción</dt><dd className="mt-1 text-sm leading-6 text-slate-800">{tramite.descripcion || "Sin descripción registrada."}</dd></div><div className="py-4"><dt className="text-sm text-slate-500">Tipo</dt><dd className="mt-1 text-sm font-medium text-slate-900">{isMaintenance ? "Mantenimiento" : "Abono"}</dd></div></dl></section>
      <aside className="space-y-4">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-medium text-slate-500">Cliente</p>{tramite.clientes ? <Link href={`/clientes/${tramite.clientes.id}`} className="mt-1 block font-semibold text-slate-950 hover:text-blue-600">{tramite.clientes.nombre}</Link> : <p className="mt-1 text-sm text-slate-600">Sin cliente asignado</p>}<p className="mt-2 text-sm text-slate-500">{tramite.clientes?.email || tramite.clientes?.telefono || "Sin contacto registrado"}</p></section>
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-medium text-slate-500">Equipo</p>{tramite.equipos ? <><Link href={`/equipos/${tramite.equipos.id}`} className="mt-1 block font-semibold text-slate-950 hover:text-blue-600">{tramite.equipos.marca} {tramite.equipos.modelo}</Link><p className="mt-2 text-sm text-slate-500">{[tramite.equipos.ubicacion, tramite.equipos.capacidad].filter(Boolean).join(" · ") || "Sin datos adicionales"}</p></> : <p className="mt-1 text-sm text-slate-600">Sin equipo asignado</p>}</section>
      </aside>
    </div>
